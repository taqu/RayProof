 ...
 */
/**
This is altered version by Takuro Sakai at 2022.
*/
...
#define TFALSE 0
#define TTRUE 1
#define LG3_MALLOC ::malloc
#define LG3_FREE ::free
...
tbool genTangSpace(const SMikkTSpaceContext* pContext, const float fAngularThreshold)
{
    ...
    piTriListIn = (int*)LG3_MALLOC(sizeof(int) * 3 * iNrTrianglesIn);
    pTriInfos = (STriInfo*)LG3_MALLOC(sizeof(STriInfo) * iNrTrianglesIn);
    if(piTriListIn == NULL || pTriInfos == NULL) {
        if(piTriListIn != NULL){
            LG3_FREE(piTriListIn);
        }
        if(pTriInfos != NULL){
            LG3_FREE(pTriInfos);
        }
    ...
    pGroups = (SGroup*)LG3_MALLOC(sizeof(SGroup) * iNrMaxGroups);
    piGroupTrianglesBuffer = (int*)LG3_MALLOC(sizeof(int) * iNrTrianglesIn * 3);
    if(pGroups == NULL || piGroupTrianglesBuffer == NULL) {
        if(pGroups != NULL){
            LG3_FREE(pGroups);
        }
        if(piGroupTrianglesBuffer != NULL){
            LG3_FREE(piGroupTrianglesBuffer);
        }
        LG3_FREE(piTriListIn);
        LG3_FREE(pTriInfos);
        return TFALSE;
    ...
    psTspace = (STSpace*)LG3_MALLOC(sizeof(STSpace) * iNrTSPaces);
    if(psTspace == NULL) {
        LG3_FREE(piTriListIn);
        LG3_FREE(pTriInfos);
        LG3_FREE(pGroups);
...
static void GenerateSharedVerticesIndexList(int piTriList_in_and_out[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn)
{
    ...
    // make allocations
    piHashTable = (int*)LG3_MALLOC(sizeof(int) * iNrTrianglesIn * 3);
    piHashCount = (int*)LG3_MALLOC(sizeof(int) * g_iCells);
    piHashOffsets = (int*)LG3_MALLOC(sizeof(int) * g_iCells);
    piHashCount2 = (int*)LG3_MALLOC(sizeof(int) * g_iCells);

    if(piHashTable == NULL || piHashCount == NULL || piHashOffsets == NULL || piHashCount2 == NULL) {
        if(piHashTable != NULL){
            LG3_FREE(piHashTable);
        }
        if(piHashCount != NULL){
            LG3_FREE(piHashCount);
        }
        if(piHashOffsets != NULL){
            LG3_FREE(piHashOffsets);
        }
        if(piHashCount2 != NULL){
            LG3_FREE(piHashCount2);
        }
    ...
    pTmpVert = (STmpVert*)LG3_MALLOC(sizeof(STmpVert) * iMaxCount);
        ...
        if(verts == 3) {
...

            {
                ...
                if(distSQ_02 < distSQ_13)
                    ...
                    bQuadDiagIs_02 = LengthSquared(vsub(P3, P1)) < LengthSquared(vsub(P2, P0)) ? TFALSE : TTRUE;
                }
...
static void InitTriInfo(STriInfo pTriInfos[], const int piTriListIn[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn)
{
    ...
    // match up edge pairs
    {
        SEdge* pEdges = (SEdge*)LG3_MALLOC(sizeof(SEdge) * iNrTrianglesIn * 3);
        if(pEdges == NULL)
                             ...
                             const SMikkTSpaceContext* pContext)
{
    ...
    pSubGroupTspace = (STSpace*)LG3_MALLOC(sizeof(STSpace) * iMaxNrFaces);
    pUniSubGroups = (SSubGroup*)LG3_MALLOC(sizeof(SSubGroup) * iMaxNrFaces);
    pTmpMembers = (int*)LG3_MALLOC(sizeof(int) * iMaxNrFaces);
    if(pSubGroupTspace == NULL || pUniSubGroups == NULL || pTmpMembers == NULL) {
        if(pSubGroupTspace != NULL){
            LG3_FREE(pSubGroupTspace);
        }
        if(pUniSubGroups != NULL){
            LG3_FREE(pUniSubGroups);
        }
        if(pTmpMembers != NULL){
            LG3_FREE(pTmpMembers);
        }
        return TFALSE;
    }
    ...
    for(g = 0; g < iNrActiveGroups; g++) {
        ...
        for(i = 0; i < pGroup->iNrFaces; i++) // triangles
        {
            ...
            if(!bFound) {
                // insert new subgroup
                int* pIndices = (int*)LG3_MALLOC(sizeof(int) * iMembers);
                if(pIndices == NULL) {
                    // clean up and return false
                    for(int t = 0; t < iUniqueSubGroups; t++){
                        LG3_FREE(pUniSubGroups[t].pTriMembers);
                    }
                    LG3_FREE(pUniSubGroups);
                    LG3_FREE(pTmpMembers);
                    LG3_FREE(pSubGroupTspace);
        ...
        for(int s = 0; s < iUniqueSubGroups; s++){
            LG3_FREE(pUniSubGroups[s].pTriMembers);
        }
